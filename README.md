...
Profiles, events, prices, and settings remain on the local computer. The app contacts Albion's Game Info endpoints, the Albion Online Data Project price endpoints, and GitHub for application updates and installer downloads.

## Application updates

The installed Windows app checks this repository's stable GitHub releases at startup and every six hours. It asks **Update Yes/No** once per new version, remembering the prompt across restarts. **Yes** downloads the installer and restarts the app to install it. **No** dismisses the prompt; **Settings → App updates** remains available to check and update later. No download starts without choosing to update. Check or download failures appear in Settings and can be retried without additional popup prompts. Updates are disabled during development.

Releases must include the NSIS installer, `latest.yml`, and installer blockmap generated by electron-builder. The release workflow uploads these together and requires the `v` tag to match `package.json`'s version. Existing installations without the updater need one manual installation of a release containing this feature.
